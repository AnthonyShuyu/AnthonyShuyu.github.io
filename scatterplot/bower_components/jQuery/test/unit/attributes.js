...
QUnit.test( "val()", function( assert ) {
	...
	assert.strictEqual(
		jQuery( "</optgroup></select><select name="select12584" id="select12584"><option value="1" disabled="disabled">1</option></select>" ).val(),
		null,
...
QUnit.test( "val() with non-matching values on dropdown list", function( assert ) {
	...
	var select6 = jQuery( "</option><select multiple id="\"select6\""><option value="\"1\"">A</option><option value="\"2\"">B</option></select>" ).appendTo( "#form" );
...
QUnit.test( "option value not trimmed when setting via parent select", function( assert ) {
	assert.expect( 1 );
	assert.equal( jQuery( "</option><select><option> 2</option></select>" ).val( "2" ).val(), "2" );
} );
...
</map></div></select></div></div></div></div></div></div></a></a>