...
QUnit.test( "jQuery.fx.prototype.cur() - <1.8 7="" back="" compat",="" function(="" assert="" )="" {="" assert.expect(="" );="" var="" div="jQuery(" "<div=""></1.8></div>" ).appendTo( "#qunit-fixture" ).css( {
			color: "#ABC",
...
QUnit.test( "Handle queue:false promises", function( assert ) {
	...
	foo.animate( {
		...
		queue: false,
		complete: function() {
			assert.ok( step++ <= 1="" 2,="" "step="" one="" or="" two"="" );="" }="" ).animate(="" {="" bottom:="" },="" duration:="" 10,="" complete:="" function()="" assert.ok(="" step=""> 2 && step < 5, "Step three or four" );
			step++;
		}
	} );
...
</span></span></div></div></div></div></div></=></div></div></div></div></div></div></div></div>